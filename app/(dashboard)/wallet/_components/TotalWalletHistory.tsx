...
const TotalWalletHistory = ({ closeTBalance }: any) => {
  ...
  const { transactions, isLoading, error, fetchSpendingHistory } = useTransactionHistory(token || "");
  ...
  return (
    <View className="absolute -top-2 gap-5 w-full">
      ...
      {isLoading ? (
        ...
        <View className="flex-1 justify-center items-center">
          <Text className="text-gray-400 text-center">No spending history found</Text>
        </View>
      ) : (
        <FlatList
          ...
          renderItem={({ item }) => (
            <View className="flex-row items-center my-2">
              <View className="flex-row items-center gap-3 flex-1">
                ...
                {item.description.toLowerCase().includes('video') || item.description.toLowerCase().includes('series') ? (
                  ...
                  <Image
                    source={
                      item.description.toLowerCase().includes('gift') || item.description.toLowerCase().includes('tip')
